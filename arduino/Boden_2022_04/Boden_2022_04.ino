...
int schwellwert = 100;  // setzen und immer wieder prüfen !!!
...
void sensorread() // liest alle 64 Sensorwerte aus und speicher sie im Puffer ab.
{
  ...
  */
  setzeMultiplexer(2);  // r3
    ...
    //Serial.println(String(analogRead(sensor[0]))+ " "+String(analogRead(sensor[1]))+ " "+String(analogRead(sensor[2]))+ " "+String(analogRead(sensor[3]))+ " ");
...
boolean abgewichen() {
  int diff = 0;
  if(weiss_winkel+8 >= weiss_winkel_save) diff = weiss_winkel - weiss_winkel_save;
  else diff = weiss_winkel - weiss_winkel_save;
  diff = abs(diff);
...
void linie_erkennen()
{
  if (zuordnungzone > 0)   // weiße Linie wird irgendwo erkannt
  {
    ...
    else      // die Linie wurde erkannt, die Savelinie ist klar definiert...
    {
      ...
      if (!abgewichen()) // Abweichung 90 Grad oder weniger    // muss noch oder Sprung -7->8 und umgekehrt
      {
...
void loop()
{
  ...
  //schreibe();
  /*if(sieht_weiss) {
    Serial.println(" abweichung : " + String(abgewichen()));
    Serial.println("     winkel : " + String(weiss_winkel));
    Serial.println("       save : " + String(weiss_winkel_save));
  }
  Serial.println();*/
